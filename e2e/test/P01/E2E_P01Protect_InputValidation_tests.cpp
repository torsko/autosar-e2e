...
class P01Protect_InputValidation : public ::testing::Test {
protected:
    void SetUp() override {
...
TEST_F(P01Protect_InputValidation, NullConfigReturnsINPUTERR_NULL) {
    uint8_t buffer[5];

    Std_ReturnType result = E2E_P01Protect(nullptr, &state_, buffer);

    EXPECT_EQ(result, E2E_E_INPUTERR_NULL);
}

TEST_F(P01Protect_InputValidation, NullStateReturnsINPUTERR_NULL) {
    uint8_t buffer[5];

    Std_ReturnType result = E2E_P01Protect(&config_, nullptr, buffer);

    EXPECT_EQ(result, E2E_E_INPUTERR_NULL);
}

TEST_F(P01Protect_InputValidation, NullDataReturnsINPUTERR_NULL) {
    Std_ReturnType result = E2E_P01Protect(&config_, &state_, nullptr);
 ...
 */
TEST_F(P01Protect_InputValidation, CounterOffsetNotNibbleAlignedReturnsINPUTERR_WRONG) {
    uint8_t buffer[5];

    config_.CounterOffset = 0;
    Std_ReturnType result = E2E_P01Protect(&config_, &state_, buffer);
    EXPECT_EQ(result, E2E_E_OK);

    config_.CounterOffset = 1;
    result = E2E_P01Protect(&config_, &state_, buffer);
 ...
 */
TEST_F(P01Protect_InputValidation, CRCOffsetNotByteAlignedReturnsINPUTERR_WRONG) {
    uint8_t buffer[5];
...
TEST_F(P01Protect_InputValidation, DataIdModeOutsideValidRangeReturnsINPUTERR_WRONG) {
    uint8_t buffer[5];
 ...
 *
 */
TEST_F(P01Protect_InputValidation, DataIdNibbleOffsetMustBeZeroUnlessDATAID_NIBBLE) {
    uint8_t buffer[5];
...
TEST_F(P01Protect_InputValidation, DataIdNibbleOffsetMustByteOrNibbleAligned) {
    uint8_t buffer[5];
    config_.DataIDMode = E2E_P01_DATAID_NIBBLE;
...
TEST_F(P01Protect_InputValidation, DataLengthMustAtLeastHoldCrcAndCounter) {
    uint8_t buffer[5];
...
TEST_F(P01Protect_InputValidation, DataLengthMustBeMultipleOfEight) {
    uint8_t buffer[5];
